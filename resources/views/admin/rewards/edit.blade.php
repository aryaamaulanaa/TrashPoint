...
@section('page_title')
    <div class="flex items-center text-gray-800 text-3xl font-bold">
        ...
        <span class="leading-none">Edit Hadiah: {{ $reward->name }}</span>
    </div>
@endsection
    ...
    <div class="form-container">
        <form action="{{ route('admin.rewards.update', $reward->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="number" id="koin_required" name="koin_required" value="{{ old('koin_required', $reward->koin_required) }}" required min="1">
                @error('koin_required')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                <label for="image">Gambar Hadiah (Opsional):</label>
                ...
                @if ($reward->image)
                    ...
                    <img src="{{ asset('storage/' . $reward->image) }}" alt="{{ $reward->name }}" class="current-image">
                @endif
            </div>
            <button type="submit" class="btn-action btn-primary">Update Hadiah</button>
