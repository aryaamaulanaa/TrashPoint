...
@section('page_title')
    <div class="flex items-center text-gray-800 text-3xl font-bold">
        ...
        <span class="leading-none">Edit Kategori Sampah: {{ $category->name }}</span>
    </div>
@endsection
    ...
    <div class="form-container">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="number" id="koin_per_kg" name="koin_per_kg" step="0.01"
                    value="{{ old('koin_per_kg', $category->koin_per_kg) }}" required>
                @error('koin_per_kg')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <button type="submit" class="btn-action btn-primary">Update Kategori</button>
